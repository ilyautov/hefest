...
```bash
make test    # офлайн-сьют (113 тестов): без Ollama, без сети, без индекса
...
FastAPI (43 эндпоинта), 18 HTML-экранов (каждый со своим `<style>`). Ретриверы: lexical /
...
2597 веществ, 25970 разделов-чанков. Реестр площадок в поставку НЕ входит (DATA-LICENSE.md §4) —
подключается через `PLANTS_FILE`. Корпус: реальные регуляторные значения
